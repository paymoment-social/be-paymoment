...
function actionResult(title: string, data: unknown, cards: Array<Record<string, any>> = []) {
  return { content: [{ type: "text" as const, text: title }], structuredContent: { type: "paymoment.action", brand: brand(), action: { title, data }, cards: cards.map(decoratePost) } };
}
