...
function styles() {
  ...
  .verified svg{width:12px;height:12px;fill:none;stroke:#17121f;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.avatar,.profile-avatar{overflow:hidden;display:block}.avatar img,.profile-avatar img{width:100%;height:100%;object-fit:contain;padding:5px}
  </style>`;
}
...
function cardMarkup(card: Card, website?: string) {
  ...
  const avatarMarkup = `<img class="avatar" src="${escapeHtml(avatar || logoUrl)}" alt="" onerror="this.onerror=null;this.src='${escapeHtml(logoUrl)}'" />`;
...
function rewardMarkup(reward: NonNullable<ToolResult["structuredContent"]>["reward"]) {
  if (!reward) return "";
  const amount = reward.granted_points ?? (reward.claimed ? 10 : reward.balance ?? 0);
  const label = reward.granted_points ? "Box earned" : reward.claimed === false ? "Already claimed" : "Box balance";
  return `<article class="reward-card"><div class="reward-icon">B</div><div><div class="reward-label">${escapeHtml(label)}</div><div class="reward-amount">${escapeHtml(amount)} <span>${escapeHtml(reward.unit || "Box")}</span></div><div class="reward-title">${escapeHtml(reward.title || "PayMoment Box")}</div></div></article>`;
}
...
function render(result?: ToolResult) {
  ...
  root.querySelectorAll<HTMLElement>(".verified").forEach((element) => {
    element.textContent = "";
    element.innerHTML = `<svg viewBox="0 0 16 16" aria-hidden="true"><path d="m4 8.2 2.4 2.3L12 5" /></svg>`;
  });
