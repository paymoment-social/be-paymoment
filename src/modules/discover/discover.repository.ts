...
export async function searchDiscover(viewerId: string, query: string, type: DiscoverType, limit: number, cursorValue?: string) {
  const db = getDb();
  const value = query ? `%${query.replace(/[%_\\]/g, "\\$&")}%` : "%";
  const hasQuery = query.trim().length > 0;
  const cursor = decodeCursor(cursorValue);
  const searchVector = sql`to_tsvector('simple', coalesce(${posts.body}, '') || ' ' || coalesce(${articles.title}, '') || ' ' || coalesce(${articles.description}, '') || ' ' || coalesce(${articles.contentText}, ''))`;
  const postScore = hasQuery ? sql<number>`ts_rank(${searchVector}, websearch_to_tsquery('simple', ${query}))` : sql<number>`extract(epoch from ${posts.publishedAt})`;
  const personScore = hasQuery ? sql<number>`similarity(coalesce(${users.displayName}, '') || ' ' || coalesce(${users.username}, ''), ${query})` : sql<number>`extract(epoch from coalesce(${users.lastSeenAt}, ${users.createdAt}))`;
  ...
  const [peopleRows, momentRows, articleRows, topicRows] = await Promise.all([
    ...
    needsMoments ? db.select({ id: posts.id, publishedAt: posts.publishedAt, score: postScore }).from(posts).leftJoin(articles, eq(articles.postId, posts.id)).where(and(eq(posts.status, "published"), isNull(posts.deletedAt), eq(posts.visibility, "public"), ne(posts.kind, "article"), sql`not exists (select 1 from user_blocks b where (b.blocker_id = ${viewerId} and b.blocked_id = ${posts.authorId}) or (b.blocker_id = ${posts.authorId} and b.blocked_id = ${viewerId}))`, sql`not exists (select 1 from user_mutes m where m.muter_id = ${viewerId} and m.muted_id = ${posts.authorId} and (m.expires_at is null or m.expires_at > now()))`, hasQuery ? sql`${searchVector} @@ websearch_to_tsquery('simple', ${query})` : undefined, type === "moments" ? postCursor : undefined)).orderBy(desc(postScore), desc(posts.publishedAt), desc(posts.id)).limit(limit + 1) : [],
    needsArticles ? db.select({ id: posts.id, publishedAt: posts.publishedAt, score: postScore }).from(posts).innerJoin(articles, eq(articles.postId, posts.id)).where(and(eq(posts.status, "published"), isNull(posts.deletedAt), eq(posts.visibility, "public"), eq(posts.kind, "article"), eq(articles.status, "published"), sql`not exists (select 1 from user_blocks b where (b.blocker_id = ${viewerId} and b.blocked_id = ${posts.authorId}) or (b.blocker_id = ${posts.authorId} and b.blocked_id = ${viewerId}))`, sql`not exists (select 1 from user_mutes m where m.muter_id = ${viewerId} and m.muted_id = ${posts.authorId} and (m.expires_at is null or m.expires_at > now()))`, hasQuery ? sql`${searchVector} @@ websearch_to_tsquery('simple', ${query})` : undefined, type === "articles" ? postCursor : undefined)).orderBy(desc(postScore), desc(posts.publishedAt), desc(posts.id)).limit(limit + 1) : [],
...
export async function listDiscoverSuggestions(query: string, limit: number) {
  ...
  const [people, topics] = await Promise.all([
    getDb().select({ displayName: users.displayName, username: users.username }).from(users).where(and(eq(users.status, "active"), isNull(users.deletedAt), or(ilike(users.displayName, value), ilike(users.username, value), sql`similarity(coalesce(${users.displayName}, '') || ' ' || coalesce(${users.username}, ''), ${query}) > 0.2`))).orderBy(desc(sql`similarity(coalesce(${users.displayName}, '') || ' ' || coalesce(${users.username}, ''), ${query})`), desc(users.lastSeenAt)).limit(limit),
    getDb().select({ label: hashtags.displayLabel, slug: hashtags.slug }).from(hashtags).where(or(ilike(hashtags.slug, value), ilike(hashtags.displayLabel, value), sql`similarity(${hashtags.slug}, ${query}) > 0.2`)).orderBy(desc(hashtags.postCount), desc(hashtags.updatedAt)).limit(limit),
  ]);
  return [
