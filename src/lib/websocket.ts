...
export type RealtimeEvent = { type: "message.created" | "message.read" | "message.requested" | "conversation.updated" | "notification.created" | "post.created" | "presence.updated" | "typing.updated"; data: Record<string, unknown>; occurred_at: string };
...
function deliverAll(event: RealtimeEvent) { sockets.forEach((userSockets) => userSockets.forEach((socket) => send(socket, event))); }
...
export async function startRealtimeSubscription() {
  ...
  subscriber.on("message", (channel, payload) => { if (channel === redisKeys.websocketBroadcastChannel()) { try { deliverAll(JSON.parse(payload) as RealtimeEvent); } catch { /* malformed broadcast payloads are ignored */ } } });
  await Promise.all([subscriber.psubscribe(redisKeys.websocketPattern()), subscriber.subscribe(redisKeys.websocketBroadcastChannel())]);
  subscribed = true;
}
...
export async function publishBroadcastEvent(type: RealtimeEvent["type"], data: Record<string, unknown>) {
  const event: RealtimeEvent = { type, data, occurred_at: new Date().toISOString() };
  await getRedisPublisher().publish(redisKeys.websocketBroadcastChannel(), JSON.stringify(event));
}
