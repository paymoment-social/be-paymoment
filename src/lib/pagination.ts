...
export type TimeCursor = { created_at: string; id: string; score?: number; ranking_at?: string };
...
export function decodeCursor(value?: string | null): TimeCursor | null {
  ...
  try {
    ...
    return { created_at: parsed.created_at, id: parsed.id, ...(parsed.score !== undefined ? { score: parsed.score } : {}), ...(parsed.ranking_at !== undefined ? { ranking_at: parsed.ranking_at } : {}) };
  } catch {
